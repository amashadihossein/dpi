...
creds_set_aws <- function(profile_name = character(0),
                          key = character(0),
                          secret = character(0)){

  if(!(length(profile_name) > 0 | length(key) > 0 & length(secret) > 0))
    stop(cli::format_error(glue::glue("Either a named profile or both AWS ",
  ...
  if(length(profile_name) > 0){
    ...
    if(class(aws_profiles) == "try-error")
      stop(cli::format_error(glue::glue("Failed to successfully retrieve AWS ",
    ...
    if(! profile_name %in%  valid_profile_names)
      stop(cli::format_error(glue::glue("{profile_name} is not a valid ",
                                        "crudential name. Valid names are ",
                                        "{paste(valid_profile_names,collapse = \"\\n \")}")))

    aws_creds <- data.frame(profile_name  = profile_name,
                            key = "",
                            secret = "")
  ...
  if(length(key) == 0 | length(secret) == 0)
    stop(cli::format_error(glue::glue("key has length {length(key)} and secret ",
                                      "has length {length(secret)}. Either ",
                                      "valid AWS (key, secret) pair or a valid",
                                      " AWS profile name is needed.")))


  aws_creds <- data.frame(profile_name  = "",
                          key = key,
                          secret = secret)
